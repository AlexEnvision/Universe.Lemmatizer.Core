...
namespace Universe.Lemmatizer.Tests.FormsApp.Helpers
{
    public static class LemmatizerMorphHelper
    {
        public static string[] TransformMessage(this ILemmatizer lemmatizer, string[] messageTokens)
        {
            MatList<string> editPhrase = new MatList<string>();
            foreach (string item in messageTokens)
            {
                ...
                if (wordFormParadigmes != null && wordFormParadigmes.Count > 0)
                {
                    ...
                    editPhrase += paradigm.Norm.ToLower(); //paradigm.GetForm(1);
                }
            }
